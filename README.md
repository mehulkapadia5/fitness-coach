...
```bash
git clone https://github.com/<your-user>/mealfix.git
cd mealfix
npm install
```
...
```bash
npx wrangler login
npx wrangler d1 create mealfix
```
...
```bash
npx wrangler d1 migrations apply mealfix --local
npx wrangler d1 migrations apply mealfix --remote
```
...
Wrangler prints your live URL, e.g. `https://mealfix.<subdomain>.workers.dev`.
...
- **Remote MCP server URL**: `https://mealfix.<your-subdomain>.workers.dev/mcp/<your-prod-token>`
...
```bash
npx wrangler d1 export mealfix --remote --output=backup-$(date +%Y%m%d).sql
```

Drop that file in iCloud/Dropbox once a month. To restore on a fresh D1: `wrangler d1 execute mealfix --remote --file=backup.sql`.
