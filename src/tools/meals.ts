...
export function registerLogMeal(
  server: McpServer,
  getDB: () => D1Database,
): void {
  server.registerTool(
    ...
    async (args) => {
      const row = await insertMeal(getDB(), {
      ...
      const time = istTimeString(new Date(row.eaten_at));
      ...
      const summary = `Logged: ${row.description}${macroSuffix} at ${time} IST`;
