    ...
    'log_workout',
    {
      ...
      inputSchema: {
        ...
        done_on: z
          ...
          .optional()
          .describe(
            'YYYY-MM-DD in IST. Defaults to today IST. Only set when the user explicitly logs a past workout.',
          ),
      },
    },
    async (args) => {
      const row = await insertWorkout(getDB(), {
        type: args.type,
      ...
      const day = istDayOfWeek(new Date(row.done_at));
      const time = istTimeString(new Date(row.done_at));
      const intensityPart = row.intensity ? ` (${row.intensity})` : '';
      const summary = `Logged: ${day} ${row.type}${intensityPart} at ${time} IST`;
